...
@Service
public class NoticeManageInterceptor implements HandlerInterceptor {
	...
	@Override
	public boolean preHandle(HttpServletRequest request, // 사용자가 보낸 정보
			HttpServletResponse response, // 사용자에게 보낼 정보
			Object handler) throws Exception {
		// 작성자 본인이라는 것은 게시글의 작성자와 현재 세션의 회원아이디가 같음을 의미
		// - 게시글 정보를 불러오려면 게시글 번호와 NoticeDao가 필요
		...
		if (isAdmin) {// 관리자
			// 삭제 페이지로 이동한다면
			if (request.getRequestURI().equals("/notice/delete")) {
				return true;
			}
		}
		if (isOwner) {// 작성자 본인
