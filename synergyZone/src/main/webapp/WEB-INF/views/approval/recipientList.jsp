							...
							<tbody v-else>
								<tr
									...
									<td
										...
										class="pointable">
										{{approval.approvalWithDrafterDto.draftTitle}}</td>
									<td><span class="pointable"
										@click="showDrafterInfoModal(index)">
											{{approval.approvalWithDrafterDto.empName}} </span></td>
									<td v-if="approval.approvalWithDrafterDto.resultCode > 0">
										-</td>
									<td v-else><span class="pointable"
									...
									<td v-else><span class="pointable"
										@click="showFinalApproverInfoModal(index)">
											{{approval.approverList[approval.approverList.length -
											1].empName }} </span></td>
									<td><span
			...
			<div class="d-flex">
				<ul class="pagination">
					<li class="page-item" @click="firstMove"
						...
						<span class="page-link"> <i class="fa-solid fa-angles-left"></i>
					</span>
					</li>
...
<script>
        Vue.createApp({
            ...
            methods:{
                async loadData(){
                    const resp = await axios.get(contextPath+"/rest/approval/recipientList");
                ...
                async move(page) {
                	 ...
                	 this.ApprovalWithPageVO.paginationVO.pageStatus = this.pageStatus; // pageStatus 추가
                	  const resp = await axios.post(contextPath+"/rest/approval/recipientMoveList", this.ApprovalWithPageVO.paginationVO);
