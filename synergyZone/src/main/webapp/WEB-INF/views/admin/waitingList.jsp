...
<div class="container-800" style="margin-left: 5%;">
   ...
   <form class="d-flex" action="${pageContext.request.contextPath}/admin/waitingList" method="get">
      ...
      <a href="${pageContext.request.contextPath}/admin/exitList" class="btn btn-outline-secondary">&nbsp;퇴사 목록</a>
...
<!--                         </td> -->
                        <td class="align-middle">
                           <div class="profile-image employee-name"
                              ...
                              data-empphone="${employeeDto.empPhone}"
                              data-empemail="${employeeDto.empEmail}"
                              data-empaddress="${employeeDto.empAddress}"
                              data-empdetailaddress="${employeeDto.empDetailAddress}"
                       ...
                       ${departmentDto.deptName}
                        ...
                        <td class="align-middle"><c:forEach var="jobDto"
                              items="${jobs}">
                       ...
                       ${jobDto.jobName}
                        ...
                        <td class="align-middle">
                           <a href="${pageContext.request.contextPath}/admin/exitCancel?empNo=${employeeDto.empNo}"
                              class="exit-cancel-button">퇴사취소</a> 
                           <a href="${pageContext.request.contextPath}/admin/finalExit?empNo=${employeeDto.empNo}"
         ...
         <div class="modal-content">
            <div class="modal-header">
               ...
               <div class="modal-body">
                  <p>
                     <strong>사원번호 :</strong> <span id="employeeNo"></span>
                  </p>
                  <p>
                     <strong>이름 :</strong> <span id="employeeName"></span>
                  </p>
                  <p>
                     <strong>전화번호 :</strong> <span id="employeePhone"></span>
                  </p>
                  <p>
                     <strong>이메일 :</strong> <span id="employeeEmail"></span>
                  </p>
                  <p>
                     <strong>주소 :</strong> <span id="employeeAddress"></span>
                  </p>
                  <p>
                     <strong>상세주소 :</strong> <span id="employeeDetailAddress"></span>
                  </p>
                  <p>
                     <strong>부서 :</strong> <span id="employeeDepartment"></span>
                  </p>
                  <!-- 부서 정보를 표시할 곳 -->
                  <p>
                     <strong>직위 :</strong> <span id="employeeJob"></span>
                  </p>
                  <!-- 직위 정보를 표시할 곳 -->
                  <!-- 정보 추가 가능 -->
               </div>
   ...
   <div class="modal fade" id="employeeInfoModal" tabindex="-1"
      role="dialog" aria-labelledby="employeeInfoModalLabel"
      aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
         <div class="modal-content">
            <div class="modal-header">
   ...
   <div style="display: flex; justify-content: center;">
      <ul class="pagination" style="width: 20%;">
         <li class="page-item ${vo.isFirst() ? 'disabled' : ''}"><a
            class="page-link"
            href="${vo.isFirst() ? '#' : pageContext.request.contextPath}/admin/waitingList?page=${vo.getPrevPage()}&sort=${vo.getSort()}${vo.getQueryString()}">&laquo;</a>
         </li>
         <c:forEach var="i" begin="${vo.getStartBlock()}"
            end="${vo.getFinishBlock()}">
            <li class="page-item"><a
               class="page-link ${vo.getPage() eq i ? 'active' : ''}"
               href="${pageContext.request.contextPath}/admin/waitingList?page=${i}&sort=${vo.getSort()}${vo.getQueryString()}">
                  <span class="text-info">${i}</span>
            </a></li>
         </c:forEach>
         <li class="page-item ${vo.isLast() ? 'disabled' : ''}"><a
            class="page-link"
            href="${vo.isLast() ? '#' : pageContext.request.contextPath}/admin/waitingList?page=${vo.getNextPage()}&sort=${vo.getSort()}${vo.getQueryString()}">
         ...
         .ready(
               function() {
                  ...
                  $('.employee-name')
                        .click(
                              function() {
                                 var employeeNo = $(this).data(
                                       'empno');
                                 var employeeName = $(this).data(
                                       'empname');
                                 var employeePhone = $(this).data(
                                       'empphone');
                                 var employeeEmail = $(this).data(
                                       'empemail');
                                 var employeeAddress = $(this).data(
                                       'empaddress');
                                 var employeeDetailAddress = $(this)
                                       .data('empdetailaddress');
                                 var attachmentNo = $(this).data(
                                       'attachmentno');
                                 var departmentName = $(this)
                                       .closest('tr').find(
                                             'td:eq(8)').text(); // 부서 정보 가져오기
                                 var jobName = $(this).closest('tr')
                                       .find('td:eq(9)').text(); // 직위 정보 가져오기
                                 ...
                                 $('#employeeAddress').text(
                                       employeeAddress);
                                 $('#employeeDetailAddress').text(
                                       employeeDetailAddress);
                                 $('#employeeDepartment').text(
                                       departmentName); // 부서 정보 설정
...
<script>
   ...
   $(document).ready(
         function() {
            // 선택된 옵션 설정
            $(
                  'select[name="column"] option[value="' + selectedColumn
                        + '"]').prop('selected', true);
         });
</script>
